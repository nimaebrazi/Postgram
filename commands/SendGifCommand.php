...
class SendGifCommand extends MessageHandler
{
    protected $name;
    ...
    public function __construct(Telegram $telegram, $update)
    {
        ...
        $this->name = Command::SEND_GIF;
        $this->description = Command::SEND_GIF_DESC;
        $this->usage = Command::SEND_GIF_USAGE;
        $this->version = Command::SEND_GIF_VERSION;
        $this->enabled = true;
        $this->public = true;
    ...
    public function execute()
    {
        ...
        if ( States::START_SEND_GIF == $user->getStateByChatId($chatId) )
        {
...
//            }
            ...
            case(States::CHANNEL_SAVED):
                ...
                if ( (strlen($content) < 200) && ($content != null || $content != "") )
                {
                    ...
                    if ( $content == Buttons::WITHOUT_CONTEXT )
                    {
                        ...
                        $this->saveGifResponse($messageId, $chatId);
                        break;
            ...
            case (States::CONTEXT_SAVED):
                ...
                if ( $this->getUpdate()->getMessage()->getDocument() != null
                    || $this->getUpdate()->getMessage()->getDocument() != ""
                )
                {
                    $post->updatePost($postData, "file_id", $this->getUpdate()->getMessage()->getDocument()->getFileId());
                    $user->updateState(States::GIF_SAVED, $chatId);
                ...
                {
                    $this->saveGifFailedResponse($messageId, $chatId);
                    break;
                }
                ...
                if ( $validator->isMinute($userResponse) )
                {
                    ...
                    )
                    {
                        if ( $userResponse >= $time->getPresentMinute() )
                        {
                            ...
                            $this->gifPostPreviewResponse($chatId, $post->getPostContent($postData));
